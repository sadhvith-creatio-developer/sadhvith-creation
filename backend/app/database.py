...
def _ensure_indexes() -> None:
    ...
    db.accounts.create_index([("role", ASCENDING)])
    # OTPs auto-expire from MongoDB at exactly `expiresAt` — no cron/cleanup job needed.
    db.otps.create_index([("expiresAt", ASCENDING)], expireAfterSeconds=0)
    db.otps.create_index([("email", ASCENDING), ("role", ASCENDING), ("purpose", ASCENDING)], unique=True)
